...
class HelloTestService:
    ...
    def find_by_id(self, entity_id):
        ...
        data = HelloTest.query.get(entity_id)
        if data is None:
            raise HelloTestServiceError(
                'hellotest {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = HelloTest.query.filter(
            HelloTest.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = HelloTest.query.get(entity_id)  # HelloTest is a mongo class
        if record is None:
            raise HelloTestServiceError(
                'hellotest {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
